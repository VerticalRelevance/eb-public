...
def disable_az(availability_zone: str,
    ...
    for vpc_id in get_vpc_ids(configuration=configuration):
        try:
            subnets = get_subnets_in_az(
                availability_zone=availability_zone, vpc_id=vpc_id, configuration=configuration
            )
            nacl_associations = get_subnets_nacl_associations(subnets=list(subnets), configuration=configuration)
            blackhole_nacl_id = __create_blackhole_nacl(vpc_id=vpc_id, configuration=configuration)
            blackhole_nacl_ids.append(blackhole_nacl_id)
            rollback_nacl = {
                **rollback_nacl,
                **apply_new_nacl(nacl_associations=nacl_associations, new_nacl_id=blackhole_nacl_id, configuration=configuration)
            }
    ...
    save_rollback_config(rollback_config={'rollback_nacl_association': rollback_nacl, 'blackhole_nacl_ids': blackhole_nacl_ids},
        filename=state_file_name, configuration=configuration)


def reenable_az(state_file_name: str = 'disable_az.json',
                configuration: Configuration = None):
    """
    Re-enables a previously disabled AZ (based on the state in S3)
    """
    if (rollback_config := load_rollback_config(state_file_name, configuration=configuration)):
        ec2 = aws_client('ec2', configuration=configuration)

        logger.info('Restoring NACL associations...')
        for nacl_association_id, nacl_id in rollback_config['rollback_nacl_associations'].items():
...
def __create_blackhole_nacl(vpc_id: str,
                            configuration: Configuration = None):
