...
def get_session():
    ...
    password = client.get_parameter( Name=PASS_PARAM, WithDecryption=True).get('Parameter').get('Value')
    
    session = requests.Session()
    session.post('https://query-url', 
                 data = {'userName': USERNAME, 'password': password}, verify=False)
    
    return session

def load_probe_absolute(testday: str, teststart: str, testend: str, 
                       sla: str, missed_percent: int) -> bool:
...
def load_probe_relative(testday: str, minutes: int, 
                       sla: str, missed_percent: int) -> bool:
...
def run_stats_collect(session, sla, teststart, testend):
    response = session.post('https://query-url', 
                 json = {
                    "name": "get-stats",
                        "params": {
                            "sla": sla,
